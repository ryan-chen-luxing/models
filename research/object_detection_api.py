...
class ObjectDetectionApi:
    ...
    def __init__(self):
        ...
        if MODEL_NAME.find('coco') != -1:
            PATH_TO_LABELS = os.path.join('object_detection/data', 'mscoco_label_map.pbtxt') ### CWH: Add object_detection path
        elif MODEL_NAME.find('kitti') != -1:
            PATH_TO_LABELS = os.path.join('object_detection/data', 'kitti_label_map.pbtxt') ### CWH: Add object_detection path

        NUM_CLASSES = 90

        # Download Model
        print('downloading model: {}'.format(DOWNLOAD_BASE + MODEL_FILE))
    ...
    def getObjects(self, image_np, threshold=0.5):
